...
def parser(file_dir,output_dir,idx, 
			namespace,page_titles,limit=None,limit2print=100):
	'''
	...
	outputfile_dir = os.path.join(output_dir,'wiki_history_parsed_%d'%(idx)) 
	...
	for pages in dump:
		if(pages.namespace == namespace and 
			pages.title.replace(' ','').lower() in page_titles):
			...
			for revisions in pages._Page__revisions:
				...
				df = df.append(row)
		if(idx >= limit & limit != None): break
		if( (limit2print != None)  & (idx % limit2print == 0)):
